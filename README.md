<div align="center">

# 🚀 Icarus-Rocket

**A student-built model rocket targeting 1 km apogee with propulsive vertical landing.**

![License](https://img.shields.io/badge/license-MIT-blue)
![Status](https://img.shields.io/badge/status-active%20development-brightgreen)

</div>

---

##  Quick Stats
- **Target Apogee:** 1,000 m AGL
- **Recovery Method:** Propulsive vertical landing (PID + TVC)
- **Project Codename:** Icarus-Rocket
- **Status:**  Early development

##  Mission
Build a thrust-vectored model rocket capable of reaching 1 km altitude 
and executing a controlled propulsive landing — a miniature analog of 
orbital booster recovery systems.

##  Repository Structure
- [`firmware/`](firmware/) — Flight computer embedded software
- [`simulation/`](simulation/) — 6-DOF trajectory & control simulations
- [`cad/`](cad/) — Mechanical design (CAD models)
- [`electronics/`](electronics/) — Schematics and PCB designs
- [`docs/`](docs/) — Engineering documentation
- [`media/`](media/) — Build photos and flight videos

##  Current Progress
- [x] Repository initialized
- [ ] Mission requirements defined
- [ ] Concept design complete
- [ ] Sub-scale test flight
- [ ] Full-scale 1 km flight with propulsive landing

##  Author
Built by **Jonathan Wigton** — aspiring aerospace engineer.

##  License
MIT — see [LICENSE](LICENSE).
